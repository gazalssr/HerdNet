...
class FolderDataset(CSVDataset):
    ...
    def _load_target(self, index: int) -> Dict[str,List[Any]]:
        ...
        ############################################################
@DATASETS.register()
class BalancedFolderDataset(FolderDataset):
    def __init__(
        self, 
        csv_file: str, 
        root_dir: str, 
        albu_transforms: Optional[list] = None,
        end_transforms: Optional[list] = None,
        num_empty_samples: int = 1000,
        num_non_empty_samples: int = 1000,
    ) -> None:
        ...
        # Initialize selected patches
        self.selected_empty_patches = []
        self.selected_non_empty_patches = []
    ...
    def _sample_patches(self):
        ...
        selected_empty_patches = random.sample(self.folder_images, self.num_empty_samples)
        self.selected_empty_patches = selected_empty_patches

        # Randomly sample non-empty patches
        non_empty_patches = [img for img in self.folder_images if img not in selected_empty_patches]
        selected_non_empty_patches = random.sample(non_empty_patches, self.num_non_empty_samples)
        self.selected_non_empty_patches = selected_non_empty_patches

    def __getitem__(self, index: int) -> Tuple[PIL.Image.Image, Dict[str, List[Any]]]:
        img_name = self.selected_patches[index]
        img_path = os.path.join(self.root_dir, img_name)
