...
###########################################
focal_loss = BinaryFocalLoss(alpha_pos=2, alpha_neg=1, beta=3, reduction='mean')
...
def get_parameter_names(model): # getting the model layers
  ...
  for l, (name,param) in enumerate(model.named_parameters()):
    ...
    param_dict[name]= l
  return param_dict
# result = get_parameter_names(dla_encoder.model)
# result = get_parameter_names(dla_encoder_decoder.model)
# print(result)
...
# Assuming 'model' is your DLAEncoder model instance
# param_dict = get_parameter_names(dla_encoder.model)
# layers_to_freeze= ['base_layer','level0','level1','level2','level3','level4','loc_head']
layers_to_freeze= []

###### Layers to Freeze ##############
#AUTO_ENCODER
param_dict= get_parameter_names(dla_encoder)

# print(param_dict)
lr = 2e-4 # learning rate


params_to_update = freeze_parts(dla_encoder,param_dict, layers_to_freeze,lr,False)
# params_to_update = freeze_parts(dla_encoder.model,param_dict, layers_to_freeze ,lr,unfreeze=False)
...
# #### Redefine dataloaders for Plots ########
# train_dataset = BinaryFolderDataset(
#     preprocess=preprocess,
#     csv_file='/herdnet/DATASETS/CAH_no_margins_30/train/Train_binary_gt.csv',
#     root_dir='/herdnet/DATASETS/CAH_no_margins_30/train/',
...
#     col='binary',
#     batch_size=32,
#     shuffle=True  # Set shuffle to True for validation during tuning
# )
...
# eval_epochs = 40
# fine_tune_epochs = 10
# def fine_tune_and_evaluate(beta, train_dataloader, val_dataloader, model, eval_epochs, fine_tune_epochs, work_dir):
...
#     print(f"Starting fine-tuning for beta={beta}")
#     for epoch in range(fine_tune_epochs):
#         fine_tuner._train(epoch)
#         print(f"Fine-tuning Epoch [{epoch+1}/{fine_tune_epochs}] completed for beta={beta}")
...
# # Beta values to test
# beta_values = [2, 2.25, 2.5, 2.75, 3, 3.25, 3.5, 3.75, 4, 4.25, 4.5, 4.75, 5]
# precisions = []
# recalls = []

# for beta in beta_values:
#     print(f"Evaluating for beta: {beta}")
#     precision, recall = fine_tune_and_evaluate(beta, train_dataloader, val_dataloader, dla_encoder, eval_epochs, fine_tune_epochs, work_dir)
...
# plt.show()
# ############## Plotalpha_pos- Recall - Precision tradeoff in Focal Loss regarding differen alpaha_pos values #######################################
# val_sampler = BinaryBatchSampler(
#     dataset=val_dataset,
#     col='binary',  
...
# eval_epochs=40
# fine_tune_epochs = 10
# def fine_tune_and_evaluate(beta, alpha_pos, train_dataloader, val_dataloader, model, eval_epochs, fine_tune_epochs, work_dir):
#     # Check if val_dataloader is not None and contains data
#     if val_dataloader is None:
#         raise ValueError("val_dataloader is None")
#     if len(val_dataloader) == 0:
...
#     focal_loss = BinaryFocalLoss(alpha_pos=alpha_pos, alpha_neg=1, beta=beta, reduction='mean')
#     losses = [{'loss': focal_loss, 'idx': 0, 'idy': 0, 'lambda': 1.0, 'name': 'focal_loss'}]
...
#     # Start fine-tuning with logging
#     print(f"Starting fine-tuning for alpha_pos={alpha_pos}")
#     for epoch in range(fine_tune_epochs):
#         fine_tuner._train(epoch)
#         print(f"Fine-tuning Epoch [{epoch+1}/{fine_tune_epochs}] completed for alpha_pos={alpha_pos}")
...
# # Beta and alpha_pos values to test
# beta = 4  
# alpha_pos_values = [1, 1.25, 1.5, 1.75, 2, 2.5, 3, 3.5, 4, 5]
# precisions = []
# recalls = []


# for alpha_pos in alpha_pos_values:
#     print(f"Evaluating for alpha_pos: {alpha_pos}")
#     precision, recall = fine_tune_and_evaluate(beta, alpha_pos, train_dataloader, val_dataloader, dla_encoder, eval_epochs, fine_tune_epochs, work_dir)
#     precisions.append(precision)
#     recalls.append(recall)
#     print(f"Results for alpha_pos={alpha_pos}: Precision: {precision}, Recall: {recall}")

# # Plot precision and recall vs alpha_pos values
# plt.figure(figsize=(12, 6))

# plt.plot(alpha_pos_values, precisions, marker='o', label='Precision', color='blue')
# plt.plot(alpha_pos_values, recalls, marker='o', label='Recall', color='orange')
# plt.title('Precision and Recall vs Alpha_pos')
# plt.xlabel('Alpha_pos')
# plt.ylabel('Score')
# plt.legend()
# plt.grid(True)

# plt.tight_layout()
# plt.savefig('/herdnet/precision_recall_vs_alpha_pos.pdf')
# plt.show()
